    ...
    <div class="right-panel" style="display: flex; flex-direction: column; padding: 20px; background-color: #f9f9f9; border-radius: 8px; height: 100vh; box-sizing: border-box;">
        <div class="container" style="height: 100%; overflow: hidden;">
            ...
            <div style="display: flex; justify-content: flex-end; margin-bottom: 10px;">
                <v-btn color="#3282B8" class="add-button" style="background-color: #3282B8; color: white; border-radius: 10px;" @click="dialogCreate = true">
                    ...
                    <span style="text-transform: none;">Nuevo Item</span>
                </v-btn>
            </div>
            <!-- Tabla personalizada -->
            <div style="background-color: white; border-radius: 8px;">
                ...
                <!-- Filas de datos -->
                <div v-for="item in inventarios" :key="item.item" style="margin-bottom: 15px; padding: 10px; border-bottom: 1px solid #ddd; color: black; border-radius: 15px; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.2)">
                        ...
                        <div v-for="header in headers" :key="header.value" style="flex: 1;">
                            ...
                            <div v-if="header.text === 'Acciones'">
                                <v-tooltip bottom color="green">
                                    ...
                                    <span> Actualiza Inventario: {{ item.item }}</span>
                                </v-tooltip>
                                <v-tooltip bottom color="red">
                                    <template #activator="{ on, attrs }">
                                        <v-btn
                                            ...
                                            color="#FF7657"
                                            v-bind="attrs"
                                            @click="deleteInventario(item.id)"
                                            v-on="on"
                                        >
                                            <v-icon>mdi-delete</v-icon>
                                        </v-btn>
                                    </template>
                                    <span> Borra Inventario: {{ item.item }}</span>
                                </v-tooltip>
                            </div>
        ...
        >
            <v-card flat elevation="1" class="rounded-lg" color="#FFFFFF">
                ...
                <v-card-text style="color: #555555; display: flex; flex-direction: column; font-size: 18px; padding: 16px;">
                    ...
                    <v-row align="center" justify="center" class="pa-2">
                        <span>Vas a borrar el Inventario.</span>
                    </v-row>
                    <v-divider class="my-2" />
                </v-card-text>
                <!-- Botones de acción -->
        ...
        >
            <crearInventario
                :inside="true"
                :update="true"
                :inventario-update="inventarioActualizar"
...
<script>
import crearInventario from './crearInventario.vue'
export default {
    components: {
        crearInventario
    },
    ...
    methods: {
        update(item) {
            this.inventarioActualizar = item
        ...
        async borralo() {
            const borrado = await this.$axios.delete(`/inventarios/delete/${this.idBorrar}`)
